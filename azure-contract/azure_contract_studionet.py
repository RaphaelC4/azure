...
# 61999 (legacy studionet) build of azure_contract.py — SAME logic, legacy
# runtime API: the Depends pin above is the only runtime this net's GenVM
# resolves, TreeMap is re-exported by `from genlayer import *` here (the
# genlayer.storage module only exists on the 61997-era py-genlayer v0.3.0),
# the contract base is gl.Contract (not gl.contract.Contract) and the
# validator timestamp is gl.message_raw (not gl.message.raw).
...
class Azure(gl.Contract):
    ...
    def reclaim_unaccepted_stake(self, case_id: str) -> str:
        ...
        # Native refund: the value leaves the contract's balance immediately
        # and is credited to the claimant when the child message finalizes.
        _Payee(Address(case["claimant"])).emit_transfer(value=amount)

        return f"Case {case_id} cancelled — stake refunded natively."
...
"""
    ...
    def _pay_winner(self, case: dict) -> None:
        ...
        # Native payout: the value leaves this contract's balance immediately
        # and is credited to the winner when the emitted child message
        # finalizes. Platform caveat (GenLayer docs): if the child
        # transaction fails, the value is not automatically returned.
...
# On payouts — native GEN transfers, no credit ledger:
#
# settle() (deadline valve), accept_verdict (joint-acceptance path) and
# reclaim_unaccepted_stake pay out with emit_transfer directly: the value
# leaves the contract's balance immediately and the winner/claimant is
# credited when the emitted child message finalizes. Platform caveat
# (GenLayer docs): if the child transaction fails, the value is not
