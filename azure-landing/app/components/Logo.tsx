export function AzureLogo({ size = 28, className = "", style, bright = false }: { size?: number; className?: string; style?: React.CSSProperties; bright?: boolean }) {
  const w = size;
  const h = Math.round((size * 256) / 196);
  return (
    <svg
      width={w}
      height={h}
      ...
      className={className}
      style={{ display: "block", filter: bright ? "drop-shadow(0 0 8px oklch(81% 0.15 88 / 0.35))" : undefined, ...style }}
      aria-hidden
    >
      <path
...
export function GenLayerMark({ size = 16, className = "" }: { size?: number; className?: string }) {
  return (
    <span aria-hidden className={className} style={{ display: "inline-flex", width: size, height: size, opacity: 0.9 }}>
      ...
      <img
        ...
        width={size}
        height={size}
        style={{ width: size, height: size, display: "block", objectFit: "contain", filter: "brightness(1.05) contrast(1.05)" }}
        draggable={false}
      />
    </span>
  );
